...
if text[0:1] == 등록:
    ...
    # 이미지를 s3에 업로드
    count -= 1
    IMAGE_OUTPUT_FILENAME = "IN-%s%d.jpg" % (System_Time.replace(":", "-"), count)
    ...
    # 카메라 프레임 읽기
        ...
        if success:
            ...
            cv2.imshow('Camera Window', frame)
            if(int(cap.get(1)) % 1 == 0):
                cv2.imwrite("./CI-%s%d.jpg" % (MODIFIED_WAVE_OUTPUTNAME.replace(":", "-"), count), frame)
                count += 1
            # ESC를 누르면 종료
            if (count == 100): 
                break
    ...
    # 이미지를 s3에 업로드
    count -= 1  
    IMAGE_OUTPUT_FILENAME = "CI-%s%d.jpg" % (MODIFIED_WAVE_OUTPUTNAME.replace(":", "-"), count)
    ...
    # 카메라 프레임 읽기
        ...
        if success:
            ...
            cv2.imshow('Camera Window', frame)
            if(int(cap.get(1)) % 1 == 0):
                cv2.imwrite("./CO-%s%d.jpg" % (MODIFIED_WAVE_OUTPUTNAME.replace(":", "-"), count), frame)
                count += 1
            # ESC를 누르면 종료
            if (count == 100): 
                break
    ...
    # 이미지를 s3에 업로드
    count -= 1  
    IMAGE_OUTPUT_FILENAME = "CO-%s%d.jpg" % (MODIFIED_WAVE_OUTPUTNAME.replace(":", "-"), count)
