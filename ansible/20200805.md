...
ansible -i INVENTORY node1 -m firewalld -a "service=http permanent=yes state=enabled" -b
ansible -i INVENTORY node1 -m seboolean -a "name=httpd_can_network_connect state=yes persistent=yes" -b
ansible -i INVENTORY node1 -m seboolean -a "name=httpd_can_network_connect_db state=yes persistent=yes" -b
ansible -i INVENTORY node1 -m seboolean -a "name=httpd_can_network_connect state=yes persistent=yes" -b

# php74 설치
ansible -i INVENTORY node1 -m yum -a "name=epel-release state=latest" -b
ansible -i INVENTORY node1 -m yum -a "name=https://rpms.remirepo.net/enterprise/remi-release-7.rpm state=latest" -b
ansible -i INVENTORY node1 -m yum -a "name=php enablerepo=remi-php74 disablerepo=remi-safe state: latest" -b
ansible -i INVENTORY node1 -m yum -a "name=php-sql enablerepo=remi-php74 disablerepo=remi-safe state: latest" -b

# HTTPD 재시작
ansible -i INVENTORY node1 -m service -a "name=httpd state=restarted enabled=true" -b
...
ansible -i INVENTORY node1 -m unarchive -a "src=/var/www/html/latest-ko_KR.tar.gz dest=/var/www/html/" -b
ansible -i INVENTORY node1 -m copy -a "src=/var/www/html/wordpress/wp-config-sample.php dest=/var/www/html/wordpress/wp-config.php 
ansible -i INVENTORY node1 -m replace -a "dest=/var/www/html/wordpress/wp-config.php find=database_name_here replace=wordpress_db" -b
ansible -i INVENTORY node1 -m replace -a "dest=/var/www/html/wordpress/wp-config.php find=username_here replace=admin" -b
ansible -i INVENTORY node1 -m replace -a "dest=/var/www/html/wordpress/wp-config.php find=password_here replace=dkagh1." -b
ansible -i INVENTORY node1 -m replace -a "dest=/var/www/html/wordpress/wp-config.php find=localhost replace=노드2의 IP" -b

# Mariadb 설치 및 활성화
ansible -i INVENTORY node2 -m yum_repository -a "name=MariaDB baseurl=http://mirror.yongbok.net/mariadb/yum/10.5/centos7-amd64 \
gpgkey=http://mirror.yongbok.net/mariadb/yum/RPM-GPG-KEY-MariaDB gpgcheck=1" -b
...
ansible -i INVENTORY node2 -m mysql_user -a "login_user=root login_password='' name=root host=localhost password=dkagh1. check_implicit_admin=yes \
priv:'*.*:ALL'" -b
...
# 익명 유저 삭제
ansible -i INVENTORY node2 -m mysql_user -a "login_user=root login_password=dkagh1. name='' host_all=yes sstate=absent" -b

# Wordpress를 위한 DB, USER, PREVILEGE 구성
...
ansible -i INVENTORY node2 -m firewalld -a "service=mysql permanent=yes state=enabled" -b
